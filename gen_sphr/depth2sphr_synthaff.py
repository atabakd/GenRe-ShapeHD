...
def plot_voxel(voxel):
  ...
  ax.set_ylabel("y")
  ax.set_zlabel("z")
  #ax.set_aspect('equal') #  https://github.com/matplotlib/matplotlib/pull/13474

  # voxel = voxel[20:80,20:80,20:80].copy()
  from skimage.measure import block_reduce
  ds_voxel = block_reduce(voxel, (2,2,2))
...
def depth2sphere(data):
  ...
  data['mask_img'] = np.where(depth_img > 0.001,
                              np.ones_like(depth_img),
...
if __name__ =='__main__':
  ...
  #import pdb; pdb.set_trace()
  import fnmatch
  #for root, dirnames, filenames in os.walk("/mnt/Extra/dsl/pix3d/debug/depth"):
  for root, dirnames, filenames in os.walk("/mnt/hdd/aff_render/depth"):
    for idx, filename in enumerate(fnmatch.filter(filenames, '*.png')):
      depth_path = os.path.join(root, filename)
      #depth_path = "/mnt/hdd/aff_render/depth/0007/001989-depth-011.png"
      data = dict()
      data['depth_path'] = depth_path
      render_path = depth_path.replace("depth", "render").replace("1.png", ".png")
      data['depth_minmax'] = np.load(
                             depth_path.replace("depth", "metadata").replace("1.png", ".npz"))["depth_minmax"]
      data['trans_mat'] = np.load(
        depth_path.replace("depth", "metadata").replace("1.png", ".npz"))["trans"]

      d_sphere = depth2sphere(data)
      #if d_sphere is None: continue
      d_sph_map, d_sph_centered_map = d_sphere
      data['voxel'] = np.load(
        depth_path.replace("depth", "voxel").replace("1.png", ".npz"))["voxel"]
      vox_sphere = vox2sphere(data)
      #if vox_sphere is None: continue
      sph_dir = root.replace("depth", "sph_maps")
      if not os.path.exists(sph_dir):
        os.makedirs(sph_dir)

      np.savez(os.path.join(sph_dir, filename.replace("depth", "spherical")),
                            depth_spherical=d_sph_map,
                            obj_spherical=vox_sphere,
                            depth_spherical_centered=d_sph_centered_map)
